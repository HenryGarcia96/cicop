...
@section('page_heading','Conceptos de salidas')
...
<div class="row">
  ...
  @endforeach
        ...
        @if(isset($rol1))
        {{Form::open(array('id'=>'frmsalidas','url' => ['salidas/edit',$rol1['id']],'method' => 'DELETE','class'=>'has-validation-callback'))}}
        @else
        {{Form::open(array('id'=>'frmsalidas','url' => 'salidas/create','method' => 'POST','class'=>'has-validation-callback'))}}
        @endif
        <input class="form-control" id='idrol' name='idrol' type="hidden" @if(isset($rol1))
            ...
            <div class="form-group">
                ...
                <input class="form-control" id='concepto' name='concepto' data-validation="required"
                @if(isset($rol1))
                    value="{{ $rol1['concepto'] }}"
                @endif
                >
            </div>
            <a href="{{url('/salidas')}}" class="btn btn-success">Nuevo</a>
            @if(Session::get('menu')['SALIDAS-G']==1)
        ...
        @section ('htable_panel_title','Lista de salidas de productos')
        ...
        <table class="table table-hover">
            ...
            @foreach ($roles as $rol)
                <tr data-id="{{$rol['id']}}">
                    ...
                    <td>
                      @if(Session::get('menu')['SALIDAS-ED']==1)
                    <a href="{{url('/salidas/show/'.$rol['id'])}}" class="btn-editar btn btn-primary"><i class="fa fa-edit"></i></a>
                    @endif
                    </td>
                    <td>
                    @if(isset($rol1) && Session::get('menu')['SALIDAS-EL']==1)
                        @if ($rol['id']!=$rol1['id'] )
                            <a href="#!" class="btn-delete btn btn-danger"><i class="fa fa-trash-o"></i></a>
                        @endif
                    @elseif (Session::get('menu')['SALIDAS-EL']==1)
                            <a href="#!" class="btn-delete btn btn-danger"><i class="fa fa-trash-o"></i></a>
                    @endif
                    </td>
                </tr>
    ...
    {{Form::open(array('url' => ['salidas/destroy','SALIDA_ID'],'method' => 'DELETE','id'=>'form-delete'))}}
    {{ Form::close() }}
</div>
@section('scripts')
...
$(function() {
    ...
    $(document).ready(function()
    {
        ...
        $('.btn-delete').click(function()
        {
            ...
            var url=form.attr('action').replace('SALIDA_ID',id);
